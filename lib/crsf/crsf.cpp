          ...
          if (id == CRSF_FRAMETYPE_RADIO_ID)
          {
            ...
            )
            {
                  ...
                  getCrossfireTelemetryValue(10, (int32_t *)&correction, 4))
              {
                if (!debugEnabled)
                {
                  dbout.printf("CRSF_FRAMETYPE_RADIO_ID...%i :: %u \n", id, SerialInBuffer[5]);
                  dbout.printf("updateInterval: %u :: ", updateInterval);
                  dbout.printf("correction: %i\n", correction);
                }
            ...
            {
              // if (debugEnabled) dbout.printf("Ping...\n");
              protocol_module_type(module_type);
              CRSF_broadcast_ping();
              // CRSF_read_param(0, 0, ELRS_ADDRESS);
            }
          }
...
void CRSF_serial_rcv(uint8_t *buffer, uint8_t num_bytes)
{
  ...
  {
    dbout.printf("CRSF FRAMETYPE: 0x%x : L:%u : \n", buffer[0], num_bytes);
  }
  else
  {
...
void parse_elrs_info(uint8_t *buffer)
{
  ...
  dbout.printf("LOCAL:: %u :: %u \n", local_info.bad_pkts, local_info.good_pkts);
