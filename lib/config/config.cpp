...
uint32_t tickTime = 0;
uint32_t tickInterval = 20000 * 100; // 0.5 sec. to check if rx or tx connect/disconnect
...
void check_link_state(uint32_t currentMicros)
{
    ...
    // dbout.printf("aileron: %d", rc_input->aileron);
    // dbout.printf(" :: elevator: %d", rc_input->elevator);
    // dbout.printf(" :: throttle: %d", rc_input->throttle);
    ...
    if (txConected > 0)
    {
        ...
        dbout.printf("bad_pkts: %u ", elrs_info.bad_pkts);
        dbout.printf("good_pkts: %u\n", elrs_info.good_pkts);
